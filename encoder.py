...
from torch_geometric.nn import GCNConv, SAGEConv, LGConv, TAGConv, JumpingKnowledge
from SGConv import SGConv, DRSGConv, SGConvJK
...
class SGC(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers,
        ...
        self.bns = nn.BatchNorm1d(hidden_channels)
        self.dropout = dropout
...
class SGCwithJK(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers,
        ...
        self.convs = torch.nn.ModuleList()
        self.convs.append(
            SGConv(in_channels, hidden_channels, K=3))
        for _ in range(num_layers - 2):
            self.convs.append(
...
class SGCWithAttention(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers,
                 dropout):
        super(SGCWithAttention, self).__init__()
        self.k = 25
        self.hidden = hidden_channels
        self.num_layer = num_layers
        self.in_channels = in_channels
